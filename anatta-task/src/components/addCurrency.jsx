...
const AddCurrency = () =>{
    ...
    const addCurrency = (e) =>{
        ...
        if(!isElePresent){                
            setFirstState([...firstSet,[filterResult[0], filterResult[1] * baseVal]])  
        }else{
    ...
    return (
        <form onSubmit={addCurrency} className='currency-form'>  
            <a href='javascript:;' onClick={currencyDropdownDisplay} className='add-currency-item button-secondary'>Add More Currency</a>                                               
            <div className="currency-selector-section">
                <select className='currency-selector' value={currencyVal} onChange = {getCurrency}>                    
                    {
                        data.map(currency => (
