...
const BaseCurrency = () =>{
    ...
    const [data,,firstSet,setFirstState,baseCurrency] = useContext(CurrencyDataContext);        
    ...
    return(
        <div className='base-curency'>
            <span className='usd-label'><i>USD - United States Dollars</i></span>
            <form>
                <label>{baseCurrency}</label>
                <input className='base-val' type="text" name='price' value={price} onChange={updatePrice} onKeyUp={updateAllPrice}/>
            </form>
        </div>
    )
}
